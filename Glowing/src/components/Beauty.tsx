import React from 'react'
import Link from './Link'

const Beauty: React.FC = () => {
    return (
        <section className="beauty">
            <div className="container beauty-products">
                <div className="row mehsullar">
                    <div className="products-left col-12 col-md-6">
                        <img src="https://glowing.g5plus.net/elementor/wp-content/uploads/2021/10/banner-21.jpg" alt="" />
                        <div className="product-overlay">
                            <h3>Empower Yourself
                            </h3>
                            <p>Get the skin you want to feel
                            </p>

                            <Link linkIcindekiYazi = "Explore More"/>
                        </div>
                    </div>
                    <div className="products-right col-12 col-md-6">
                        <div className="beauty-cards">
                            <div className="beauty-card">
                                <div className="beauty-first">
                                    <div className="card">
                                        <div className="card-image">
                                            <img src="https://glowing.g5plus.net/elementor/wp-content/uploads/2021/10/product-11-3-400x533.jpg" alt="" />
                                            <div className="icons-overlay">
                                            <i className="fa-solid fa-bag-shopping"></i>
                                            <i className="fa-solid fa-eye"></i>
                                            <i className="fa-solid fa-star"></i>
                                            <i className="fa-brands fa-nfc-directional"></i>
                                            </div>
                                            <div className="card-title">
                                                <div className="card-text">$10.00 – $20.00
                                                </div>
                                                <div className="card-name">Shield Conditioner</div>
                                                <div className="rate">

                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="card">
                                        <div className="card-image">
                                            <img src="https://glowing.g5plus.net/elementor/wp-content/uploads/2021/10/product-13-4-400x533.jpg" alt="" />
                                            <div className="icons-overlay">
                                            <i className="fa-solid fa-bag-shopping"></i>
                                            <i className="fa-solid fa-eye"></i>
                                            <i className="fa-solid fa-star"></i>
                                            <i className="fa-brands fa-nfc-directional"></i>
                                            </div>
                                            <div className="card-title">
                                                <div className="card-text"> $20.00
                                                </div>
                                                <div className="card-name">Perfecting facial oi</div>
                                                <div className="rate">

                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="card">
                                        <div className="card-image">
                                            <img src="https://glowing.g5plus.net/elementor/wp-content/uploads/2021/10/product-10-3-400x533.jpg" alt="" />
                                            <div className="icons-overlay">
                                            <i className="fa-solid fa-bag-shopping"></i>
                                            <i className="fa-solid fa-eye"></i>
                                            <i className="fa-solid fa-star"></i>
                                            <i className="fa-brands fa-nfc-directional"></i>
                                            </div>
                                            <div className="card-title">
                                                <div className="card-text">$ 23.00
                                                </div>
                                                <div className="card-name">Enriched Hand & Body Wash</div>
                                                <div className="rate">

                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="beauty-second">
                                    <div className="card">
                                        <div className="card-image">
                                            <img src="https://glowing.g5plus.net/elementor/wp-content/uploads/2021/10/product-12-3-400x533.jpg" alt="" />
                                            <div className="icons-overlay">
                                            <i className="fa-solid fa-bag-shopping"></i>
                                            <i className="fa-solid fa-eye"></i>
                                            <i className="fa-solid fa-star"></i>
                                            <i className="fa-brands fa-nfc-directional"></i>
                                            </div>
                                            <div className="card-title">
                                                <div className="card-text">$ 45.00
                                                </div>
                                                <div className="card-name">Shield Shampoo</div>
                                                <div className="rate">

                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="card">
                                        <div className="card-image">
                                            <img src="https://glowing.g5plus.net/elementor/wp-content/uploads/2021/10/product-08-04-400x533.jpg" alt="" />
                                            <div className="icons-overlay">
                                            <i className="fa-solid fa-bag-shopping"></i>
                                            <i className="fa-solid fa-eye"></i>
                                            <i className="fa-solid fa-star"></i>
                                            <i className="fa-brands fa-nfc-directional"></i>
                                            </div>
                                            <div className="card-title">
                                                <div className="card-text">$ 90.00
                                                </div>
                                                <div className="card-name">Enriched Hand Wash</div>
                                                <div className="rate">

                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="card">
                                        <div className="card-image">
                                            <img src="https://glowing.g5plus.net/elementor/wp-content/uploads/2021/10/product-07-04-400x533.jpg" alt="" />
                                            <div className="icons-overlay">
                                            <i className="fa-solid fa-bag-shopping"></i>
                                            <i className="fa-solid fa-eye"></i>
                                            <i className="fa-solid fa-star"></i>
                                            <i className="fa-brands fa-nfc-directional"></i>
                                            </div>
                                            <div className="card-title">
                                                <div className="card-text">$18.00 -$ 90.00
                                                </div>
                                                <div className="card-name">Enriched Duo</div>
                                                <div className="rate">

                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                    <i className="fa-solid fa-star"></i>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>

                    </div>
                </div>
            </div>
        </section>
    )
}

export default Beauty